...
| `make down` | 停止・削除（**DBデータは残る**） |
| `make clean` | 停止し、DBボリュームごと完全削除（**リセット**） |
...
- 日常の停止は `docker compose stop`（データ保持）、片付けは `make down`（データ保持）。
  DB を初期化したいときだけ `make clean`（ボリュームごと削除）→その後 `make migrate`。
- DB の確認・操作方法（Prisma Studio / psql / Prisma Client error の対処）は
  `docs/db-の使い方.md` を参照。
